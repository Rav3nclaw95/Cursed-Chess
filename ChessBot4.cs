...
public class ChessBot4 : MonoBehaviour
{
    ...
    public float EvaluateBoard(int[,,] AnalyzedIntBoard, int[,] AnalyzedSideBoard, int pastX, int pastY, int CurX, int CurY){
        ...
        if (AnalyzedIntBoard[CurY, CurX, 0] != 1 && pastY + 1 != CurY && pastY - 1 != CurX){
    ...
    public int[,] MoveSidePiece(int[,,] ChangedIntBoard, int[,] ChangedSideBoard, int x, int y, int dx, int dy){
        ChangedSideBoard[y, x] = ChangedSideBoard[dy, dx];
        ChangedSideBoard[dy, dx] = 0;
        ...
        for (int i = 7; i >= 0; i--){
            for (int t = 7; t >= 0; t--)
            {
                if (CurrentObjBoard[i, t] != null){
                    Side1 = CurrentObjBoard[i, t].GetComponent<PawnMove>().GetColor();
                }else{
    ...
    //Need to check for problems within logic for adding to heat board
    ...
    //Need to update making moves to add seperate board move functions
                    ...
                    if (y == 6){
                       if (AnalyzedSideBoard[y - 2, x] == 0){
                            ...
                            if (Evaluation > SentData[0] && Side == 1){
                                ...
                                SentData[2] = y - 2;
                                SentData[1] = x;
                                Debug.Log("Chose to move piece 2 spaces");
                            }else if (Evaluation < SentData[0] && Side == 2){
                                SentData[0] = Evaluation;
                                SentData[2] = y - 2;
                                SentData[1] = x;
                                ...
                                Debug.Log("Chose to not move piece 2 spaces");
                            }
                        }
                    }
                    ...
                    for (int m = 0; m <= 7; m++){
                        if (UsedSideBoard[y, x] == GivenSide && UsedIntBoard[y, x, m] != 0){
                            if (UsedSideBoard[y, x] == GivenSide){
                                int TempSide = GivenSide;
                                Side = GivenSide;
                                GivenInfo = NextLegalMove(UsedIntBoard, UsedSideBoard, y, x, m, GivenSide);
                                Debug.Log("Given info from NextLegalMove: Start Pos: ("+ x +", "+ y +") To Pos: ("+ GivenInfo[1] +", "+ GivenInfo[2] +") with a Eval of "+ GivenInfo[0]);
                                CopyIntBoard(UsedIntBoard, SendingIntBoard);
                                CopySideBoard(UsedSideBoard, SendingSideBoard);
                                SendingIntBoard = MoveIntPiece(SendingIntBoard, SendingSideBoard, x, y, (int) GivenInfo[1], (int) GivenInfo[2]);
                                SendingSideBoard = MoveSidePiece(SendingIntBoard, SendingSideBoard, x, y, (int) GivenInfo[1], (int) GivenInfo[2]);
